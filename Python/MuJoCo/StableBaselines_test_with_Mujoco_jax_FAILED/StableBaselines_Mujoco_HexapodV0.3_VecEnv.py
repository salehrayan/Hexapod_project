...
class HexapodV0_3VecEnv(VecEnv):
    ...
    def step_wait(self):
        ...
        return (np.asarray(obs), np.asarray(rewards[:, 0]), np.asarray(dones[:, 0]), infos)
    ...
    def _reset_done_envs_wrapper(self, key, mjx_model, current_mjx_datas, previous_obs, done_indexes):
        ...
        new_obs, new_mjx_datas = self._reset_done_envs(keys[:-1], mjx_model, current_mjx_datas, previous_obs,
                                                       done_indexes)
    ...
    def _reset_done_envs(self, key_input, mjx_model, mjx_data, obs_input, done):
        new_obs, mjx_data, _ = jax.lax.cond(done, self._reset_single, lambda *args: (obs_input, mjx_data, key_input),
                                            key_input, mjx_model)
    ...
    def step(self, action, mjx_model, mjx_data, num_steps):
        ...
        reward = correctDirectionReward + tiltReward - 1 * terminated
...
model = PPO('MlpPolicy', verbose=1, env=vec_env, device='cpu')
# model = PPO.load(path=r'C:\Users\ASUS\Downloads\best_model.zip')
model.set_logger(new_logger)

model.learn(total_timesteps=10, progress_bar=True, callback=callback_list)
